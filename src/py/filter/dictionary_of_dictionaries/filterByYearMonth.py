...
# returns all items of a dictionary that match a given year and month
...
def filterByYearMonth(whichList, whichMonth):
    ...
    for key in whichList:
        ...
        if whichList[key]['date'].startswith(whichMonth):
            # assign the entire profile object to our new dictionary using the same key
            result[key] = whichList[key]

    # return the new dictionary containing only the matching items
    return result

####

if __name__ == '__main__':
    # call the function with our people dictionary and the month '2024-05'
